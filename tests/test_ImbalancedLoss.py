...
from src.loss.ImbalancedLoss import ImbalancedLoss
...
def test_1():
    ...
    _ = ImbalancedLoss("cpu", 0.2)

    with pytest.raises(Exception):
        ImbalancedLoss("cpu", -1, 2)

    with pytest.raises(Exception):
        ImbalancedLoss("cpu", -1, 0.5)

    with pytest.raises(Exception):
        ImbalancedLoss("cpu", -1, 0.5)

    with pytest.raises(Exception):
        ImbalancedLoss(torch.device("cpu"), 0.5, -0.5)

    with pytest.raises(Exception):
        ImbalancedLoss(-2, 0.5, -0.5)


def test_2():
    """Check `sigmoid_loss` for correct properties."""
    loss = ImbalancedLoss("cpu", 0.5)
