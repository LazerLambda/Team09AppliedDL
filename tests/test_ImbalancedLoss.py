...
from src import ImbalancedLoss as il
...
def test_1():
    ...
    _ = il.ImbalancedLoss("cpu", 0.2)

    with pytest.raises(Exception):
        il.ImbalancedLoss("cpu", -1, 2)

    with pytest.raises(Exception):
        il.ImbalancedLoss("cpu", -1, 0.5)

    with pytest.raises(Exception):
        il.ImbalancedLoss("cpu", -1, 0.5)

    with pytest.raises(Exception):
        il.ImbalancedLoss(torch.device("cpu"), 0.5, -0.5)

    with pytest.raises(Exception):
        il.ImbalancedLoss(-2, 0.5, -0.5)


def test_2():
    """Check `sigmoid_loss` for correct properties."""
    loss = il.ImbalancedLoss("cpu", 0.5)
...
def test_5():
    """Test  `nn_balancePN` for correct properties."""
    loss = il.ImbalancedLoss("cpu", 0.5)
...
def test_6():
    ...
    loss = il.ImbalancedLoss("cpu", 0.5)
    assert loss(torch.rand(n), torch.sign(torch.empty(n).random_(2) - 0.5)) >= 0
